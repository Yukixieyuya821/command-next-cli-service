...
根据提示填写内容，会在项目根目录下生成 `command-next.config.js` 文件，初始化配置只会生成 `dev` (开发环境)、`test` (测试环境)、`prod` (生产环境) 三个配置，再有其他配置可参考模板自行配置。


#### 2.3 手动创建或修改配置文件

在项目根目录下手动创建 `command-next.config.js` 文件，复制以下代码按情况修改即可。
